    ...
    <main>
      <div style={{ 
        ...
        <div style={{ maxWidth: '1250px', margin: '0 auto', position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1.5rem' }}>
          <div>
            <h1 style={{ fontSize: '2.5rem', fontWeight: 'bold', letterSpacing: '-0.02em', color: '#fff', margin: 0, marginBottom: '0.75rem' }}>
              Community Leaderboard
            </h1>
            <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', maxWidth: '44rem', margin: 0 }}>
              See who's leading the charge across our servers. Search by player name and click headers to order stats dynamically.
            </p>
          </div>
      ...
      <div style={{ padding: '3rem 2rem', maxWidth: '1250px', margin: '0 auto' }}>
        <div style={{ background: 'rgba(18, 18, 22, 0.7)', border: '1px solid var(--border-light)', borderRadius: '0.75rem', overflow: 'hidden', backdropFilter: 'blur(12px)' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid var(--border-light)', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 'bold', color: 'var(--text-muted)', userSelect: 'none' }}>
                  <th onClick={() => handleSort('player')} style={{ padding: '1.2rem 1.5rem', cursor: 'pointer' }}>
                    PLAYER {renderSortIcon('player')}
                  </th>
                  <th onClick={() => handleSort('rank')} style={{ padding: '1.2rem 1rem', cursor: 'pointer' }}>
                    RANK {renderSortIcon('rank')}
                  </th>
                  <th onClick={() => handleSort('playtime')} style={{ padding: '1.2rem 1rem', cursor: 'pointer' }}>
                    PLAYTIME {renderSortIcon('playtime')}
                  </th>
                  <th onClick={() => handleSort('last_server')} style={{ padding: '1.2rem 1rem', cursor: 'pointer' }}>
                    LAST SERVER {renderSortIcon('last_server')}
                  </th>
                  <th onClick={() => handleSort('last_online')} style={{ padding: '1.2rem 1rem', cursor: 'pointer' }}>
                    LAST ONLINE {renderSortIcon('last_online')}
                  </th>
                  <th onClick={() => handleSort('votes')} style={{ padding: '1.2rem 1rem', cursor: 'pointer' }}>
                    VOTES {renderSortIcon('votes')}
                  </th>
                  <th onClick={() => handleSort('discord')} style={{ padding: '1.2rem 1.5rem', cursor: 'pointer' }}>
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={p.uuid} onClick={() => window.location.href = `/player/${p.uuid}`} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background-color 0.2s', cursor: 'pointer' }} className="hover-row">
                      ...
                      {/* PLAYTIME */}
                      <td style={{ padding: '1rem', color: 'rgba(255,255,255,0.85)', fontFamily: 'monospace', fontSize: '0.9rem' }}>
                        <span style={{ opacity: 0.5, marginRight: '0.3rem' }}>🕒</span>{formatPlaytime(p.playtime)}
                      </td>

                      {/* LAST SERVER */}
                      <td style={{ padding: '1rem' }}>
                        <span style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', padding: '0.35rem 0.75rem', borderRadius: '0.5rem', fontSize: '0.8rem', color: '#cbd5e1', whiteSpace: 'nowrap' }}>
                          {serverName}
                        </span>
                      </td>
                      ...
                      {/* LAST ONLINE */}
                      <td style={{ padding: '1rem' }}>
                        <span style={{ color: onlineInfo.isOnline ? '#4ade80' : 'var(--text-muted)', fontWeight: onlineInfo.isOnline ? 'bold' : 'normal', fontSize: '0.85rem' }}>
                          {onlineInfo.text}
                        </span>
                      </td>

                      {/* VOTES */}
                      <td style={{ padding: '1rem', color: '#fbbf24', fontWeight: 'bold', fontFamily: 'monospace', fontSize: '0.95rem' }}>
                        ★ {p.votes || 0}
                      </td>

                      {/* DISCORD */}
                      <td style={{ padding: '1rem 1.5rem' }}>
                        <span style={{ background: p.discord ? 'rgba(88, 101, 242, 0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${p.discord ? 'rgba(88, 101, 242, 0.35)' : 'rgba(255,255,255,0.08)'}`, color: p.discord ? '#a5b4fc' : 'var(--text-muted)', padding: '0.3rem 0.65rem', borderRadius: '0.5rem', display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem', whiteSpace: 'nowrap' }}>
